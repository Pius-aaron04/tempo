...
export const Dashboard: React.FC<DashboardProps> = () => {
    ...
    const [topProject7Days, setTopProject7Days] = React.useState({ name: '-', value: 0 });
    const [topLang7Days, setTopLang7Days] = React.useState({ name: '-', value: 0 });
    ...
    React.useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                ...
                const trends = await window.tempo.request({ type: 'query_trend', groupBy: 'project', days: 7 });
                ...
                setTotalDuration7Days(total7);

                const sortedProjs = Object.entries(projAgg).sort((a, b) => b[1] - a[1]);
                if (sortedProjs.length > 0) setTopProject7Days({ name: sortedProjs[0][0], value: sortedProjs[0][1] });

                const sortedLangs = Object.entries(langAgg).sort((a, b) => b[1] - a[1]);
                if (sortedLangs.length > 0) setTopLang7Days({ name: sortedLangs[0][0], value: sortedLangs[0][1] });
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
            ...
            <h3 style={{ margin: '0 0 10px 0', color: '#555' }}>Last 7 Days Overview</h3>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '20px' }}>
                <div style={cardStyle}>
                    <div style={iconContainerStyle}><Clock size={24} color="#0088FE" /></div>
                    <div>
                        <div style={labelStyle}>Total Time</div>
                        <div style={valueStyle}>{formatTime(totalDuration7Days)}</div>
                    </div>
                </div>
                <div style={cardStyle}>
                    ...
                    <div>
                        ...
                        <div style={valueStyle}>{formatName(topLang7Days.name)}</div>
                        <div style={{ fontSize: '0.8em', color: '#999' }}>{formatTime(topLang7Days.value)}</div>
                    </div>
                </div>
                <div style={cardStyle}>
                    ...
                    <div>
                        ...
                        <div style={valueStyle}>{formatName(topProject7Days.name)}</div>
                        <div style={{ fontSize: '0.8em', color: '#999' }}>{formatTime(topProject7Days.value)}</div>
                    </div>
                </div>
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(500px, 1fr))', gap: '20px' }}>
                ...
                <div style={chartCardStyle}>
                    <h3 style={chartTitleStyle}>Time by Project (Last 7 Days)</h3>
                ...
                {/* Language Mix */}
                <div style={chartCardStyle}>
                    <h3 style={chartTitleStyle}>Languages (Last 7 Days)</h3>
                    <div style={{ height: '300px' }}>
                        <ResponsiveContainer width="100%" height="100%">
                            <PieChart>
                                <Pie
            ...
            <div style={chartCardStyle}>
                <h3 style={chartTitleStyle}>Activities for Today</h3>
